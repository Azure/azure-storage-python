...
from .._http import HTTPError, HTTPRequest, HTTPResponse
...
class _BatchClient(_HTTPClient):
    ...
    def get_request_partition_key(self, request):
        ...
        if request.method == 'POST':
            doc = loads(request.body.decode('utf-8'))
            part_key = doc.get('PartitionKey')
            if part_key is None:
    ...
    def get_request_row_key(self, request):
        ...
        if request.method == 'POST':
            ...
            if pos1 == -1 or pos2 == -1:
                raise AzureBatchValidationError(_ERROR_CANNOT_FIND_ROW_KEY)
            return uri[pos1 + len('RowKey=\''):pos2]
